...
vx_GlBuffer vx_glbuffer_new(vx_GlBufferDescriptor* descriptor) {
...
void vx_glbuffer_bind(vx_GlBuffer* buffer) {
    VX_NULL_ASSERT(buffer);

    glBindBuffer(buffer->descriptor.type, buffer->id);
}

void vx_glbuffer_data(vx_GlBuffer* buffer, const void* data_ptr, usize data_size) {
...
void vx_glbuffer_free(vx_GlBuffer* buffer) {
    VX_NULL_ASSERT(buffer);
